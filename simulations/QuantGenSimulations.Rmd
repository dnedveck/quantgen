...
exdf1 <- data.frame(family= c(rep("a",9), rep("b",9), rep("c",9)),
                    offspring= c(1:9, 1:9, 1:9)
    )
...
```{r}
sigma_w_a <- var(exdf1$trait[which(exdf1$family=="a")]); sigma_w_a
sigma_w_b <- var(exdf1$trait[which(exdf1$family=="b")]); sigma_w_b
sigma_w_c <- var(exdf1$trait[which(exdf1$family=="c")]); sigma_w_c
```
...
```{r}
t_a <- sigma_B / (sigma_B + sigma_w_a); t_a
t_b <- sigma_B / (sigma_B + sigma_w_b); t_b
t_c <- sigma_B / (sigma_B + sigma_w_c); t_c
```
...
pframe <- data.frame(mat = mat_hold,
                         pat = pat_hold,
                         mid = mid_hold,
                         family = fam_hold,
                         offspring = off_hold)
